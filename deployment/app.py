...
def churn_prediction(tenure, citytier, warehousetohome, gender, hourspendonapp, numberofdeviceregistered, satisfactionscore, maritalstatus, numberofaddress, complain, orderamounthikefromlastyear, couponused, ordercount, daysincelastorder, cashbackamount):
    with open("deployment/models/churn_prediction_model.pkl", "rb") as f:
        model = pickle.load(f)

    with open("deployment/models/columns.json", "r") as f:
...
def index_page():
    if request.method == 'POST':
        ...
        # Get prediction
        output_probab = round((churn_prediction(*form_data)) *100, 2)

        pred = "Leave" if output_probab > 40 else "Stay"
